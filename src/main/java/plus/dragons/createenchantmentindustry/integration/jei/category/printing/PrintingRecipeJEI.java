...
public interface PrintingRecipeJEI {
    @Internal
    BiMap<Identifier, Type> TYPE_BY_ID = HashBiMap.create();
    @Internal
    BiMap<Type, Identifier> ID_BY_TYPE = TYPE_BY_ID.inverse();
    Codec<Type> TYPE_CODEC = Identifier.CODEC.xmap(TYPE_BY_ID::get, ID_BY_TYPE::get);

    static Type register(Identifier id, Type type) {
        TYPE_BY_ID.put(id.withPrefix("printing/"), type);
        return type;
    }

    static Type register(Identifier id, MapCodec<? extends PrintingRecipeJEI> codec) {
