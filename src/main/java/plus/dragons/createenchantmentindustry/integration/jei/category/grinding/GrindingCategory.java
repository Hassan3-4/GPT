...
import static plus.dragons.createenchantmentindustry.integration.jei.category.CEIJeiHelper.*;
...
public class GrindingCategory implements IRecipeCategory<RecipeHolder<GrindingRecipe>> {
    ...
    private final IDrawable icon = new IconRenderer(CEIBlocks.GRINDSTONE_DRAIN.asStack());
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, RecipeHolder<GrindingRecipe> holder, IFocusGroup focuses) {
        ...
        if (!fluidIngredients.isEmpty()) {
            CEIJeiHelper.addFluidSlot(builder, 27, 51, fluidIngredients.getFirst());
        }
        ...
        if (!fluidResults.isEmpty()) {
            CEIJeiHelper.addFluidSlot(builder, 130, 32, fluidResults.getFirst());
            i++;
        }
