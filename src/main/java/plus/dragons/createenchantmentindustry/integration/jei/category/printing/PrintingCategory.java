...
import static plus.dragons.createenchantmentindustry.integration.jei.category.CEIJeiHelper.getRenderedSlot;
...
public class PrintingCategory implements IRecipeCategory<PrintingRecipeJEI> {
    ...
    private final IDrawable icon = new IconRenderer(CEIBlocks.PRINTER.asStack());
    ...
    @Override
    public @Nullable Identifier getRegistryName(PrintingRecipeJEI recipe) {
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, PrintingRecipeJEI recipe, IFocusGroup focuses) {
        ...
        recipe.setBase(base);
        var template = builder.addSlot(RecipeIngredientRole.INPUT, 51, 5)
                .setBackground(getRenderedSlot(), -1, -1);
        recipe.setTemplate(template);
        var fluid = builder.addSlot(RecipeIngredientRole.INPUT, 27, 32)
                .setBackground(getRenderedSlot(), -1, -1)
                .setFluidRenderer(1, false, 16, 16)
                .addRichTooltipCallback(CreateCategory.POTION);
    ...
    public void draw(PrintingRecipeJEI recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics graphics, double mouseX, double mouseY) {
        ...
        var fluid = recipeSlotsView.getSlotViews().get(2)
                ...
                .map(ingredient -> new FluidStack(
                        ingredient.getFluidVariant().getFluid(),
                        (int) ingredient.getAmount(),
                        ingredient.getFluidVariant().getComponents()))
