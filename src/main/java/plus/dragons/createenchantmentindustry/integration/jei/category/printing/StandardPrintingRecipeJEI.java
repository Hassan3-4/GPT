...
public class StandardPrintingRecipeJEI implements PrintingRecipeJEI {
    ...
    public void setFluid(IRecipeSlotBuilder slot) {
        ...
        fluid.getMatchingFluidStacks().forEach(stack ->
                slot.addFluidStack(stack.getFluid(), stack.getAmount(), stack.getComponentChanges()));
    }

    @Override
    public void setOutput(IRecipeSlotBuilder slot) {
        slot.addItemStack(recipe.value().getRollableResults().getFirst().create());
    }
