...
public class ExperienceLightningCharger {
    ...
    public static Optional<BlockPos> findStrikeTarget(Level level, BlockPos pos) {
        Optional<BlockPos> target = findStrikeTargetAt(level, pos);
        if (target.isPresent())
            return target;

        // Some 1.21.11 lightning sources position the entity one block above the
        // vanilla strike position. Accept that coordinate without widening the
        // search beyond the struck column. A rod at the exact position has
        // already defined its backing block, so do not reinterpret its underside.
        if (level.getBlockState(pos).is(BlazeExperienceBlockEntity.LIGHTNING_ROD_BLOCKS))
            return Optional.empty();
        return findStrikeTargetAt(level, pos.below());
    }

    private static Optional<BlockPos> findStrikeTargetAt(Level level, BlockPos pos) {
