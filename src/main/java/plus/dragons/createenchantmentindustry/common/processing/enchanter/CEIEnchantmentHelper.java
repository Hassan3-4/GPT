...
public class CEIEnchantmentHelper {
    ...
    /** Mirrors vanilla's component selection plus CEI's enchanting templates. */
    public static DataComponentType<ItemEnchantments> getEnchantmentComponentType(ItemStack stack) {
        return stack.is(Items.ENCHANTED_BOOK) || stack.getItem() instanceof EnchantingTemplateItem
                ? DataComponents.STORED_ENCHANTMENTS
                : DataComponents.ENCHANTMENTS;
    }

    /** Reads the enchantment component appropriate for books and CEI templates. */
    public static ItemEnchantments getEnchantments(ItemStack stack) {
        return stack.getOrDefault(getEnchantmentComponentType(stack), ItemEnchantments.EMPTY);
    }

    /**
     * Writes to the component appropriate for the destination stack.  CEI's
     * core processing paths use this explicitly instead of depending on an
     * EnchantmentHelper mixin to redirect template data.
     */
    public static void setEnchantments(ItemStack stack, ItemEnchantments enchantments) {
        stack.set(getEnchantmentComponentType(stack), enchantments);
    }

    public static void addEnchantment(ItemStack stack, Holder<Enchantment> enchantment, int level) {
        var mutable = new ItemEnchantments.Mutable(getEnchantments(stack));
        mutable.upgrade(enchantment, level);
        setEnchantments(stack, mutable.toImmutable());
    }
    ...
    public static List<EnchantmentInstance> selectEnchantments(RandomSource random, int adjustedLevel, List<EnchantmentInstance> available, boolean special) {
        // Compatibility filtering mutates its input; keep the preview list reusable
        // for the block's next tick and for subsequent items.
    ...
    public static int levelExtension(Holder<Enchantment> enchantment) {
        var result = CEIDataMaps.SUPER_ENCHANTING_LEVEL_EXTENSION.get(enchantment);
