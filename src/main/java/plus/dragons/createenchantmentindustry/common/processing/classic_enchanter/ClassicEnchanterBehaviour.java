...
public class ClassicEnchanterBehaviour extends ServerFilteringBehaviour {
    /** Create Fly resolves client filter packets through this shared server type. */
    @SuppressWarnings({"unchecked", "rawtypes"})
    public static final BehaviourType<ClassicEnchanterBehaviour> TYPE =
    ...
    public ItemStack getResult(ItemStack stack) {
        ...
        var apply = WeightedRandom.getRandomItem(
                ...
                availableEnchantment.stream()
                        ...
                        .toList(),
                EnchantmentInstance::weight).get();
        ...
        var removedEnchantments = new ItemEnchantments.Mutable(stack.getOrDefault(
                CEIEnchantmentHelper.getEnchantmentComponentType(stack), ItemEnchantments.EMPTY));
        removedEnchantments.set(apply.enchantment(), 0);
        CEIEnchantmentHelper.setEnchantments(result, removedEnchantments.toImmutable());
        CEIEnchantmentHelper.addEnchantment(result, apply.enchantment(), applyLevel);
    ...
    private List<Object2IntMap.Entry<Holder<Enchantment>>> filterAvailableEnchantment(ItemStack stack) {
        ...
        return targetEnchantment.entrySet().stream()
                .filter(entry -> {
                    if (!entry.getKey().value().isSupportedItem(stack)) return false;
