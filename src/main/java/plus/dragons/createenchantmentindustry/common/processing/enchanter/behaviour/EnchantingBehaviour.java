...
public class EnchantingBehaviour {
    ...
    protected List<EnchantmentInstance> getAvailableEnchantments(Level level, ItemStack stack, boolean special) {
        ...
        var possible = level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT)
                .get(enchantmentTag)
                .stream()
                .flatMap(HolderSet::stream)
                // NeoForge tag JSON supports a non-vanilla `remove` member.
                // Fabric does not, so preserve CEI's two super-enchanting
                // subtractions explicitly: configured exclusions and curses.
                .filter(enchantment -> !special
                        || !enchantment.is(CEIEnchantments.MOD_TAGS.enchantingExclusive))
                .filter(enchantment -> !special || !enchantment.is(EnchantmentTags.CURSE))
                .filter(enchantment -> enchantment.value().isPrimaryItem(stack));
    ...
    public ItemStack getResult(Level level, ItemStack stack, RandomSource random, boolean special) {
        ...
        ItemStack result = stack.is(Items.BOOK) ? stack.transmuteCopy(Items.ENCHANTED_BOOK) : stack.copy();
        enchantments.forEach(enchantment -> CEIEnchantmentHelper.addEnchantment(
                result, enchantment.enchantment(), enchantment.level()));
        return result;
    }
