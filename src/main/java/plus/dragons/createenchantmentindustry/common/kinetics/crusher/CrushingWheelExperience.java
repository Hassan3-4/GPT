...
public class CrushingWheelExperience {
    ...
    public static void dropForCrushedEntity(CrushingWheelControllerBlockEntity crusher, LivingEntity entity) {
        ...
        ItemEntity expItem = new ItemEntity(serverLevel, outputPosition.x(), outputPosition.y(), outputPosition.z(), new ItemStack(AllItems.EXP_NUGGET, nuggets));
        expItem.setDeltaMovement(getOutputSpeed(crusher));
        AllSynchedDatas.BYPASS_CRUSHING_WHEEL.set(expItem, java.util.Optional.of(crusher.getBlockPos()));
    ...
    private static Vec3 getOutputPosition(CrushingWheelControllerBlockEntity crusher) {
        Vec3 centerPos = Vec3.atCenterOf(crusher.getBlockPos());
