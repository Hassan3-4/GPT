...
/** Mutable payload shared by CEI's Create 6.0.10-style processing recipes. */
public class ProcessingRecipeParams {
    ...
    public List<Ingredient> ingredients() { return List.copyOf(ingredients); }
    public List<ProcessingOutput> results() { return List.copyOf(results); }
    public List<FluidIngredient> fluidIngredients() { return List.copyOf(fluidIngredients); }
    public List<FluidStack> fluidResults() { return fluidResults.stream().map(FluidStack::copy).toList(); }
    public int processingDuration() { return processingDuration; }
    public HeatCondition requiredHeat() { return requiredHeat; }

    public Ingredient requireSingleItemIngredient() {
        if (ingredients.size() != 1) throw new IllegalStateException("Recipe " + id + " requires exactly one item ingredient");
        return ingredients.getFirst();
    }

    public ProcessingOutput requireSingleItemOutput() {
        if (results.size() != 1) throw new IllegalStateException("Recipe " + id + " requires exactly one item output");
        return results.getFirst();
    }

    public net.minecraft.world.item.ItemStack requireSingleItemResult() { return requireSingleItemOutput().create(); }

    public FluidIngredient requireSingleFluidIngredient() {
        if (fluidIngredients.size() != 1) throw new IllegalStateException("Recipe " + id + " requires exactly one fluid ingredient");
        return fluidIngredients.getFirst();
    }

    public FluidStack requireSingleFluidResult() {
        if (fluidResults.size() != 1) throw new IllegalStateException("Recipe " + id + " requires exactly one fluid output");
