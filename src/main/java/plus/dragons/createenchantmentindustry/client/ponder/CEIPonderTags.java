...
public class CEIPonderTags {
    ...
    public static void register(PonderTagRegistrationHelper<Identifier> helper) {
        PonderTagRegistrationHelper<ItemLike> entryHelper = helper.withKeyFunction(RegisteredObjectsHelper::getKeyOrThrow);
        ...
        entryHelper.addToTag(EXPERIENCE_APPLIANCES)
                ...
                .add(AllBlocks.SPOUT)
                .add(CEIBlocks.MECHANICAL_GRINDSTONE.get())
                .add(CEIBlocks.EXPERIENCE_HATCH.get())
                .add(CEIBlocks.EXPERIENCE_LANTERN.get())
                .add(CEIBlocks.BLAZE_ENCHANTER.get())
                .add(CEIBlocks.BLAZE_FORGER.get())
                .add(CEIBlocks.PRINTER.get());
        ...
        BuiltInRegistries.BLOCK.get(Identifier.fromNamespaceAndPath("create_dragons_plus", "fluid_hatch"))
                .ifPresent(fluidHatch -> entryHelper.addToTag(EXPERIENCE_APPLIANCES).add(fluidHatch.value()));

        if (CEIConfig.features().classicBlazeEnchanter.get()) {
            entryHelper.addToTag(EXPERIENCE_APPLIANCES)
                    .add(CEIBlocks.CLASSIC_BLAZE_ENCHANTER.get());
            entryHelper.addToTag(SUPER_EXPERIENCE_APPLIANCES)
                    .add(CEIBlocks.CLASSIC_BLAZE_ENCHANTER.get());
            entryHelper.addToTag(ARM_TARGETS)
                    .add(CEIBlocks.CLASSIC_BLAZE_ENCHANTER.get());
