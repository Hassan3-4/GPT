...
public class IntegrationIngredient {
    public static Ingredient of(String mod, String name) {
        return of(Identifier.fromNamespaceAndPath(mod, name));
    }

    public static Ingredient of(Identifier location) {
        return Ingredient.fromValues(Stream.of(new Value(location)));
    }

    public static Ingredient of(Identifier... locations) {
        return Ingredient.fromValues(Arrays.stream(locations).map(Value::new));
    }

    public record Value(Identifier location) implements Ingredient.Value {
        public static final MapCodec<Value> MAP_CODEC = Identifier.CODEC.fieldOf("item")
