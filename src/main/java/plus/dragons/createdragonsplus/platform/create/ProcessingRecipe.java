...
public abstract class ProcessingRecipe<T extends RecipeInput, P extends ProcessingRecipeParams>
    ...
    private void validate() {
        if (ingredients.size() > getMaxInputCount()
                || results.size() > getMaxOutputCount()
                || fluidIngredients.size() > getMaxFluidInputCount()
                || fluidResults.size() > getMaxFluidOutputCount()) {
    ...
    public List<ItemStack> getRollableResultsAsItemStacks() {
        return results.stream().map(ProcessingOutput::create).toList();
    }

    public void enforceNextResult(Supplier<ItemStack> result) {
        forcedResult = result;
    }

    public List<ItemStack> rollResults(RandomSource random) {
        return rollResults(random, results);
    }
    ...
    public static <P extends ProcessingRecipeParams, R extends ProcessingRecipe<?, P>> StreamCodec<RegistryFriendlyByteBuf, R> streamCodec(
            Factory<P, R> factory, StreamCodec<RegistryFriendlyByteBuf, P> paramsCodec) {
